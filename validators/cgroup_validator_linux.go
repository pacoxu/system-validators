...
func getUnifiedMountpoint() (string, error, error) {
	isCgroupfs, err := isMountPointCgroupfs(defaultUnifiedMountpoint)
	if isCgroupfs {
		return defaultUnifiedMountpoint, err, nil
	}
	mountPath, err := parseMountsFile(mountsFilePath)
	return mountPath, err, fmt.Errorf("%q is not cgroupfs: %v", defaultUnifiedMountpoint, err)
}

func isMountPointCgroupfs(path string) (bool, error) {
	...
	for scanner.Scan() {
		...
		// example fields: `28 22 0:25 / /sys/fs/cgroup rw,nosuid,nodev,noexec,relatime shared:4 - cgroup2 cgroup2 rw,seclabel`
		...
		if len(fields) >= 9 {
			if fields[5] == path {
				...
				if len(fields)-i <= 3 {
					return false, fmt.Errorf("expect 3 fields after `-` in mount info file %q", path)
				}
				return strings.Contains(fields[i+2], "cgroup"), nil
			}
		}
	}
	return false, fmt.Errorf("path %q is not cgroupfs", path)
}

func parseMountsFile(path string) (string, error) {
	f, err := os.Open(path)
	if err != nil {
		return "", err
	}
...
func (c *CgroupsValidator) Validate(spec SysSpec) (warns, errs []error) {
	...
	unifiedMountpoint, err, warn := getUnifiedMountpoint()
	if warn != nil {
...
// checkCgroupV2Freeze checks if the freezer controller is enabled in Linux kernels 5.2, we can check the existence of cgroup.freeze.
func checkCgroupV2Freeze(unifiedMountpoint string) (isCgroupfs bool, warn error) {
	...
	_, warn = os.Stat(filepath.Join(tmpDir, "cgroup.freeze"))
	if os.IsNotExist(warn) {
		return
	} else if warn != nil {
		// if the err is not NotExist error, it means that `cgroup.freeze` exists.
		isCgroupfs = true
		warn = fmt.Errorf("error stat cgroup.freeze file under %q: %v", tmpDir, warn)
		return
	}
	isCgroupfs = true
	return
